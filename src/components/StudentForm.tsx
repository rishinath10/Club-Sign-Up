...
export const StudentForm: React.FC<StudentFormProps> = ({ clubs, seatCounts, onSubmitted }) => {
  ...
  if (submittedData) {
    return (
      ...
      >
        ...
        >
          <h2 className="text-2xl font-bold text-brand-emerald-900 mb-2">You're Signed Up!</h2>
          <p className="text-brand-emerald-600 mb-6">
        ...
        >
          ...
          <div className="flex justify-between items-center py-1 border-b border-stone-200/60">
            ...
            <span className="font-semibold text-brand-emerald-900">{submittedData.class}</span>
          </div>
          <div className="flex justify-between items-center py-1 border-b border-stone-200/60">
            <span className="text-brand-emerald-500 font-medium">Club Selected</span>
            <span className="font-bold text-brand-turmeric-700 bg-brand-turmeric-50 px-2.5 py-0.5 rounded-md border border-brand-turmeric-100">
              {submittedData.clubName}
            </span>
          </div>
          <div className="flex justify-between items-center py-1">
            <span className="text-brand-emerald-500 font-medium">Registration Time</span>
  ...
  return (
    <div className="bg-white rounded-2xl border border-stone-200 p-6 md:p-8 shadow-xs">
      <div className="mb-6">
        <h2 className="text-xl font-bold text-brand-emerald-900 flex items-center gap-2">
          <Sparkles className="w-5 h-5 text-brand-turmeric-500" />
          Student Information
        </h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <label className="block text-xs font-semibold text-brand-emerald-700 uppercase tracking-wider flex items-center gap-1.5">
              <Users className="w-3.5 h-3.5 text-brand-emerald-400" />
              Select Cocurricular Club <span className="text-red-500">*</span>
            </label>
            <span className="text-xs text-brand-emerald-500 font-medium">Max 1 club per student</span>
          </div>
                ...
                <div
                  ...
                  className={`relative p-4 rounded-xl border-2 transition-all duration-150 select-none ${
                    isFull
                      ...
                      : isSelected
                      ? 'bg-brand-emerald-50/80 border-brand-emerald-900 shadow-xs cursor-pointer'
                      : 'bg-white border-stone-200 hover:border-brand-emerald-400 cursor-pointer'
                  }`}
                >
                  <div className="flex items-center justify-between gap-3 mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="font-semibold text-brand-emerald-900 text-base block leading-tight">
                          {club.name}
                        </span>
                        {club.description && (
                          <span className="text-xs text-brand-emerald-500 block mt-0.5">
                            {club.description}
                          </span>
                        )}
                      </div>
                    ...
                    <div className="text-right shrink-0">
                      {isFull ? (
                        ...
                        <span className="text-xs font-semibold text-brand-emerald-600 bg-stone-100 px-2.5 py-1 rounded-md border border-stone-200">
        ...
        <button
          ...
          className={`w-full py-3.5 px-6 rounded-xl font-bold text-sm text-white flex items-center justify-center gap-2 transition-all cursor-pointer shadow-sm ${
            isSubmitting || !selectedClubId
              ...
              : 'bg-brand-emerald-900 hover:bg-brand-emerald-800 active:scale-[0.99]'
          }`}
        >
          {isSubmitting ? (
            <>
